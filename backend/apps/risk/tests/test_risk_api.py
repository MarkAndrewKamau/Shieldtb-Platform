...
@pytest.mark.django_db
def test_risk_assessment_list_can_filter_by_patient():
    ...
    patient = Patient.objects.create(given_name="Amina", family_name="Otieno", facility=facility)
    other_patient = Patient.objects.create(
        given_name="Brian",
        family_name="Ouma",
        facility=facility,
    ...
    response = client.get("/api/v1/risk-assessments/", {"patient": patient.id})
